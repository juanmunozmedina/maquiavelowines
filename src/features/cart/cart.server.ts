...
export async function expandCartData(cartData: CartData): Promise<Cart> {
	...
	if (!productsResponse.data) {
		throw new Error('No se pudieron recuperar los productos', { cause: productsResponse.error });
	}
...
export function expandCartDataFromProducts(cartData: CartData, products: Product[]) {
	return cartData.items
		.map((item) => {
			...
			if (!product) {
				console.warn(`Producto no encontrado para variante ${item.productVariantId}`);
				return;
			}
