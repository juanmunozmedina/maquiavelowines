		...
		<Show when={deleteMutation.isIdle}>
			<div class={`flex items-start gap-8 ${props.class ?? ''}`}>
				...
				<div class="flex flex-1 flex-col">
					<p class="mt-auto py-2 text-lg/none font-medium text-slate-700 dark:text-theme-base-100">
						{props.item.productVariant.product.name}
					</p>

					<Show when={Object.values(props.item.productVariant.options).length > 0}>
						<p class="-mt-[3px] font-medium text-slate-500 dark:text-theme-base-100">
							{Object.values(props.item.productVariant.options).join(' • ')}
						</p>
					</Show>

					{/* this translation is for visual vertical centering */}
					<p class="-translate-y-0.5 py-2 font-medium leading-none text-slate-600 dark:text-theme-base-100">
						<ProductPrice
							price={
				...
				<button
					type="button"
					class="-my-2 self-end p-2 text-slate-600 hover:text-slate-700 dark:text-theme-base-100 dark:hover:text-theme-base-500"
					onClick={() => deleteMutation.mutate()}
				>
					<RiSystemDeleteBinLine class="size-6" />
					<span class="sr-only">Quitar artículo</span>
				</button>
			</div>
		</Show>
	);
