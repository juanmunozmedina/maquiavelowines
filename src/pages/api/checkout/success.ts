...
export const GET: APIRoute = async (context) => {
	...
	if (session.status !== 'complete') {
		return new Response('Sesión no completa', { status: 400 });
	}

	if (session.line_items == null) {
		return new Response('No se encontraron líneas de pedido de sesión', { status: 400 });
	}
...
async function createOrderFromStripe(
	...
	if (!customerResponse.data) {
		throw new Error(`Error inesperado al crear cliente. ${customerResponse.response.status}`);
	}
	...
	if (!order) {
		throw new Error(`Error inesperado al crear el pedido. ${orderResponse.response.status}`);
	}
